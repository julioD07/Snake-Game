...
public class Snake extends GameObject {
	...
	public Snake(char symbol, int xStartingLocation, int yStartingLocation) {
		...
		setSymbol(symbol);
		//? Seteamos la posición inicial de la serpiente desde el metodo heredado de GameObject
	...
	public void moveLeft(GameScreen screen, Snake snake, Food food) {
		...
		if (willSnakeHitWall(snake, screen)) {
            //! Realiza acciones específicas cuando la serpiente choca con la pared al moverse a la derecha
		...
		if (screen.getScreenMatrix()[snake.getY()][snake.getX() - 1] == '*') {
			...
			snake.setPuntaje((short) (snake.getPuntaje() + 1));
		}
		snake.setX(getX() - 1);
	...
	public boolean willSnakeHitWall(Snake snake, GameScreen screen) {
		...
		// Actualiza la nueva posición según la dirección del movimiento (por ejemplo, si es izquierda, resta 1 a la coordenada X)
		// Asegúrate de tener métodos en tu clase Snake para obtener la dirección del movimiento actual
		// (por ejemplo, snake.getDirection() podría devolver "LEFT", "RIGHT", "UP", o "DOWN")
